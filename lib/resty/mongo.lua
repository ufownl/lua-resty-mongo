...
local mongo_conn = require "resty.mongo.conn"
...
local function dispatch_reply(so)
  local len_reply, err = so:receive(4)
  if not len_reply then
    return nil, nil, err
  end
  local reply, err = so:receive(driver.length(len_reply))
  if not reply then
    return nil, nil, err
  end
  local result = {}
...
function mongo.client( conf  )
  ...
  local ok, err = obj.__sock:connect()  -- try connect only  once
  if not ok then
    return nil, err
  end
  return obj
...
bulkWrite.update = function(nsindex, doc)
  local args = {"update", nsindex, "filter", bson_encode(doc.filter), "updateMods", bson_encode(doc.update), "multi", doc.multi or false, "upsert", doc.upsert or false}
  filter(args, "hit", doc.hit)
...
function auth_method:auth_mongodb_cr(user,password)
  local password = ngx.md5(string.format("%s:mongo:%s",user,password))
  local result= self:runCommand "getnonce"
  if result.ok ~=1 then
    return false
  end

  local key =  ngx.md5(string.format("%s%s%s",result.nonce,user,password))
...
local function salt_password(password, salt, iter)
  salt = salt .. "\0\0\0\1"
  local output = ngx.hmac_sha1(password, salt)
  local inter = output
  for i=2,iter do
    inter = ngx.hmac_sha1(password, inter)
    output = driver.xor_dig(output, inter)
  end
  return output
end
...
function auth_method:auth_scram_sha1(username,password)
  ...
  local nonce = ngx.encode_base64(random.bytes(8))
  ...
  if string.sub(rnonce, 1, 12) ~= nonce then
    ngx.log(ngx.ERR, "Server returned an invalid nonce.")
    return false
  end
  local without_proof = "c=biws,r=" .. rnonce
  local pbkdf2_key = ngx.md5(string.format("%s:mongo:%s",username,password))
  local salted_pass = salt_password(pbkdf2_key, ngx.decode_base64(salt), iterations)
  ...
  local client_key_xor_sig = driver.xor_dig(client_key, client_sig)
  ...
  if parsed_t['v'] ~= server_sig then
    ngx.log(ngx.ERR, "Server returned an invalid signature.")
    return false
  end
  if not r.done then
    ...
    if not r.done then
      ngx.log(ngx.ERR, "SASL conversation failed to complete.")
      return false
    end
  end
  return true
...
function mongo_db:runCommand(cmd,cmd_v,...)
  ...
  -- we must hold  res  (res.data),  because  res.document is  a lightuserdata, it's a  pointer  to the string (res.data)
  local res, err =  sock:request(pack, request_id)
  if not res then
    return {ok = 0, errmsg = err}
  end
  return bson_decode(res.document)
end
